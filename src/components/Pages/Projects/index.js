...
const Project = () => {
        ...
        [
            {
                img: cosmoImage,
                title: "Cosmo",
                body: "An interpreter for a self-designed, fully turing complete language, a scripting language for the Comrade discord bot",
    ...
    const projectBlock = projects.map(row => {
        return (
        <div className="row" key={row[0].key + "" + row[1].key}>
           <ProjectDisplay image={row[0].img} title={row[0].title} body={row[0].body} link={row[0].link} key={row[0].key} />
           <ProjectDisplay image={row[1].img} title={row[1].title} body={row[1].body} link={row[1].link} key={row[1].key} />
        </div>
    )});
    ...
    return (
        <div>
            <div>
                ...
                <svg className="project-curve" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320"><path fill="#373737" d="M0,96L80,112C160,128,320,160,480,144C640,128,800,64,960,58.7C1120,53,1280,107,1360,133.3L1440,160L1440,320L1360,320C1280,320,1120,320,960,320C800,320,640,320,480,320C320,320,160,320,80,320L0,320Z"></path></svg>
            </div>
